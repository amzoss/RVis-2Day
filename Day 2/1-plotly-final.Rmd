...
# Hint: https://plotly.com/r/text-and-annotations/#styling-text
...
git_experience %>%
  ...
  mutate(first_vcs = as_factor(first_vcs) %>% 
           fct_infreq()) %>%
  count(first_vcs) %>%
  plot_ly(x = ~first_vcs, y = ~n, color = ~ first_vcs, type = 'bar')
...
git_experience %>% count(year_vcs, first_vcs) %>%
  plot_ly(x = ~year_vcs, y = ~n, color = ~first_vcs, type = 'scatter', mode = 'lines')
...
# hint: you can calculate a linear model with the following code: 
#   fitted(lm(Inclusiveness.index.2020~Z.score.Religion))

plot_ly(scatter, x = ~Z.score.Religion) %>%
  add_markers(y = ~Inclusiveness.index.2020, color = ~Continent) %>%
  add_lines(y = ~fitted(lm(Inclusiveness.index.2020~Z.score.Religion)), 
            showlegend = FALSE) %>%
  add_text(data = most_inclusive_country, x = ~Z.score.Religion, 
           y = ~Inclusiveness.index.2020, text = ~Country, textposition="bottom center",
           showlegend = FALSE)
